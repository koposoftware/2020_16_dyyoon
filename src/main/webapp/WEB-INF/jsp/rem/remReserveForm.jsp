...
<body>
				...
				<form:form name="remForm" commandName="remittanceVO" method="post" onsubmit="return true">
					...
					<table class="table table-sm">
						<tr>
							<th>예약 송금 날짜</th>
							<td>
								<form:input path="remDate" id="datepicker1" class="form-control"/>
							</td>
						</tr>
						<tr>
							<th>보내는 분</th>
						...
						<tr>
							...
							<td>
								<div class="input-group">
									...
									<form:select class="custom-select" path="infoNo" id="reserveForm-infoNo-select">
									...
									<div class="input-group-append">
										<button class="btn btn-info" type="button" disabled="disabled"
											id="reserveForm-infoNo-btn" data-toggle="modal"
					...
					<table class="table table-sm">
							...
							<td>
								<div class="input-group mb-2">
									...
									<form:input path="amount" type="text" class="form-control"
										placeholder="0.00" id="krwAmountInput-res" />
								</div>
						...
						<tr>
							<th>송금수수료</th>
							<td>
								<form:input class="form-control" path="remCharge" disabled="true" value="3000"/>
							</td>
						</tr>
						<tr>
							<th>중계수수료</th>
							<td>
								<form:input class="form-control" path="interCharge" disabled="true" value="23000"/>
							</td>
						</tr>
						<tr>
							<th>전신료</th>
							<td>
								<form:input class="form-control" path="cableCharge" disabled="true" value="5000"/>
							</td>
						</tr>
						<tr>
							<th>총출금금액(원화)</th>
	...
	<script type="text/javascript">
		...
		$('#krwAmountInput-res').keyup(function() {
			var amount = parseFloat($(this).val());
			var currencyCode = $('#reserveForm-infoNo-select').children('option:selected').data('currency')
			...
			if(currencyCode !== undefined ){
				$.ajax({
					...
					success : function(data) {
						...
						if($('#krwAmountInput-res').val().length > 0 || rate != 0){
							$('#otherAmountInput-res').val((amount / rate).toFixed(2))
						}else{
							$('#otherAmountInput-res').val(0.00)
						}
					},
					error : function() {
						alert('실패')
		...
		$('#otherAmountInput-res').keyup(function() {
			var amount = parseFloat($(this).val());
			var currencyCode = $('#reserveForm-infoNo-select').children('option:selected').data('currency')
			...
			if(currencyCode !== undefined ){
				$.ajax({
					...
					success : function(data) {
						var rate = parseFloat(data);
						if($('#otherAmountInput-res').val().length > 0 || rate != 0){
							$('#krwAmountInput-res').val((amount * rate).toFixed(2))
		...
		$('#reserveForm-infoNo-select').change(
				function() {
					var optionValue = $('#reserveForm-infoNo-select').children(
							'option:selected').val();
					if (optionValue == '') {
						$('#reserveForm-infoNo-btn').attr('disabled', 'disabled')
					} else {
						$('#reserveForm-infoNo-btn').removeAttr('disabled');
						var currencyCode = $('#reserveForm-infoNo-select').children('option:selected').data('currency')
		...
		$('#reserveForm-infoNo-btn').on('click',
				function() {
					var infoNo = $('#reserveForm-infoNo-select').children('option:selected').val();
